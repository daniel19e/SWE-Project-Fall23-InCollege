    ...
    while True:
        ...
        for job in all_jobs:
            if job["id"] == int(detail_job):
                ...
                if (
                    job["firstname"] == user_info["firstname"]
                    and job["lastname"] == user_info["lastname"]
                ):
            ...
            if (
                ...
                print(
                    f"Job ID: {job['id']}, Job Title: {job['title']}, Status: {application_status}"
                )
                continue
...
def apply_jobs_interns(student_info, job_id):
    ...
    if (
        job["firstname"] == student_info["firstname"]
        and job["lastname"] == student_info["lastname"]
    ):
        clear_terminal()
...
def try_deleting_job(user_info):
    select = input(
        "Enter the ID of the job you would like to remove (or X to cancel): "
    )
    ...
    if re.match(r"^[0-9]+$", select):
        ...
        for job in all_jobs:
            if (
                job["id"] == int(select)
                and job["firstname"] == user_info["firstname"]
                and job["lastname"] == user_info["lastname"]
            ):
