...
class DatabaseObject:
  ...
  def remove_application(self, job_id, student_id):
    self.cursor.execute("DELETE FROM job_applications WHERE job_id = ? AND student_id = ?", (job_id, student_id,))
    self.connection.commit()
  ...
  def add_saved_job(self, job_id, student_id):
    self.cursor.execute('''INSERT INTO job_saved (job_id, student_id) VALUES (?, ?)''', (job_id, student_id))
    self.connection.commit()

  def get_saved_jobs(self, student_id):
    self.cursor.execute("SELECT job_id FROM job_saved WHERE student_id = ?", (student_id,))
    return [job[0] for job in self.cursor.fetchall()]
  
  def remove_saved_job(self, job_id, student_id):
    self.cursor.execute("DELETE FROM job_saved WHERE job_id = ? AND student_id = ?", (job_id, student_id))
    self.connection.commit()
