import pytest
from jobs import try_posting_job, view_jobs_interns, my_job_postings, try_deleting_job, display_applied_jobs, try_deleting_job, handle_job_deletion, display_saved_jobs
import database
...
def test_try_deleting_another_users_job(capsys):
    ...
    db.add_new_job_post("Another", "User", "Title", "Description", "Employer", "Location", "50000")
...
def test_apply_jobs_interns(mock_add_applications, mock_already_applied, mock_get_job_by_id, capsys):
...
def test_display_applied_jobs(capsys):
    with patch('database.DatabaseObject.get_jobs', return_value=[{'id': 1, 'title': 'Test Job'}]), \
...
def test_handle_job_deletion(capsys):
    with patch('database.DatabaseObject.get_jobs', return_value=[]), \
         patch('database.DatabaseObject.get_applications_of_student', return_value=[1]), \
         patch('database.DatabaseObject.get_saved_jobs', return_value=[]):
        handle_job_deletion(mock_user_info)
        out, err = capsys.readouterr()
        assert '[Notification] - One or more jobs you have applied for or saved, have been deleted.' in out
        assert 'Job ID #1' in out


def test_display_saved_jobs(capsys):
    with patch('database.DatabaseObject.get_jobs', return_value=[{'id': 1, 'title': 'Test Job'}]), \
         patch('database.DatabaseObject.get_saved_jobs', return_value=[1]), \
         patch('builtins.input', return_value='0'):
...
def test_try_deleting_job(capsys):
    ...
    with patch('builtins.input', return_value='1'), \
         patch('database.DatabaseObject.get_jobs', return_value=[{'id': 1, 'title': 'Test Job', 'firstname': 'John', 'lastname': 'Doe'}]):
        try_deleting_job(mock_user_info)
        out, err = capsys.readouterr()
