...
def general_link():
    ...
    display_link_page("General links", general_link_names, general_links_functs)
    ...
    if user:
        ...
        if choice == "1":
            db.get_cursor().execute(
                "UPDATE college_students SET receive_emails = ? WHERE username = ?",
                (not user[6], get_current_username()),
            )
            db.get_connection().commit()
        if choice == "2":
            db.get_cursor().execute(
                "UPDATE college_students SET receive_sms = ? WHERE username = ?",
                (not user[7], get_current_username()),
            )
            db.get_connection().commit()
        if choice == "3":
            db.get_cursor().execute(
                "UPDATE college_students SET targeted_ads = ? WHERE username = ?",
                (not user[8], get_current_username()),
            )
            db.get_connection().commit()
        show_guest_controls()
    else:
...
def show_languages():
    ...
    if user:
        ...
        if choice == "1":
            db.get_cursor().execute(
                "UPDATE college_students SET language = 'english' WHERE username = ?",
                (get_current_username(),),
            )
            db.get_connection().commit()
        if choice == "2":
            db.get_cursor().execute(
                "UPDATE college_students SET language = 'spanish' WHERE username = ?",
                (get_current_username(),),
            )
            db.get_connection().commit()
        show_languages()
...
def show_copyright_notice():
    ...
    print(
        "© 2023 InCollege, Inc. All rights reserved. The content, graphics, and other elements of this website are protected under copyright law and may not be reproduced, distributed, transmitted, displayed, published, or broadcast without the prior written permission of InCollege, Inc. or in the case of third party materials, the owner of that content. Unauthorized use or infringement may result in legal action."
    )
    input("Enter any input to go back: ")
...
def show_user_agreement():
    ...
    print(
        "By using the InCollege website and its associated services, you agree to comply with all applicable laws and our terms of service. You are responsible for maintaining the confidentiality of your account and for any activities that occur under your account. InCollege, Inc. reserves the right to modify or terminate services, content, or user accounts at its discretion."
    )
    input("Enter any input to go back: ")
...
def show_brand_policy():
    ...
    print("Brand Policy")
    print(
        "Copyright © 2023 InCollege, Inc.; all content is protected by law, and unauthorized use or reproduction without our express permission is prohibited."
    )
    input("Enter any input to go back: ")
...
def show_incollege_important_links():
    ...
    display_link_page("Important links", important_link_names, important_links_functs)
...
def show_friend_requests():
    ...
    while True:
        ...
        if len(pending_requests) > 0:
            ...
            print("\n-------------------------------------")
            print(
                "You can accept friend request # by typing a# or reject by typing r#\n"
            )
        else:
